...
def string_to_binary(message):
    """converts a string message into binary with constant lenght 8 bits"""
    return(''.join(format(ord(char), '08b') for char in message))
    #ord converts every caracter to binary using Unicode
    #format with parameter '08b' make Unicode values have a constant lenght = 8
    ...
    for y in range(height):
        for x in range(width):
            ...
            if channels == 1:
                ...
                pixel = list(image.getpixel((x, y))) # converts tuple into list to make it mutable
            ...
            for color_channel in range(channels):  # 3 if RGB channels or 1 if gray-scale image
                if data_index < len(binary_message):
                    ...
                    data_index += 1
            image.putpixel((x, y), tuple(pixel)) #putpixel modify a pixel by the values of a tuple
    ...
    for y in range(height):
        for x in range(width):
            ...
            for color_channel in range(channels):  # RGB channels
                binary_message += str(pixel[color_channel] & 1) #get the last bit of every pixel value
